...
namespace matcher {
/**
 * @brief Contains the interface for adding regexes and matching a string with
 * the trie
 *
 ...
 */
template <typename RegexData, typename char_t = char> class RegexMatcher;
} // namespace matcher
...
/**
 * @brief Contains the characters and additional attributes for wildcard symbols
 * and Null symbols
 *
 */
...
struct Limits {
  ...
  bool is_allowed_to_repeat() const {
    return !max.has_value() ||
           (max.value() > 0 && this->min <= this->max.value());
  }
};
...
template <typename T, typename Node> struct EdgeInfo {
  std::map<T, std::optional<std::list<Limits>::iterator>>
      paths; // each path may have different requirements for how many times
             // should the edge be repeated.
  Node *to;
};
...
template <typename RegexData, typename char_t> class Node {
   ...
   *
   */
  std::map<symbol<char_t>, EdgeInfo<RegexData, Node<RegexData, char_t>>>
      neighbours;
   ...
   * @return false if there is no node with this symbol as direct neighbour to
   * this node
   */
  ...
  /**
   * @brief Adds a child node to the current one and marks the connection as
   * part of a given regex match
   *
   * @param child  Existing node
   * @param regex  Regex data that is being used to indentify the regex that the
   * edge is part of
   * @param limits Pointer to the shared limit of the edge (nullptr if no limit
   * is applied)
   */
  void connect_with(
      Node<RegexData, char_t> *child, RegexData regex,
      std::optional<std::list<Limits>::iterator> limits = std::nullopt);

  /**
   * @brief Matches a string with all regexes and returns the identified of the
   * one that matches
   *
   * @param text string that is being tried to be matched with any of the added
   * regexes
   * @tparam ConstIterator const iterator in a container
   * @return std::vector<RegexData> set of unique identifiers of the regexes
  ...
  template <typename ConstIterator>
  std::vector<RegexData> match_helper(ConstIterator, ConstIterator,
                                      const std::vector<RegexData> &, Node *);

#ifdef DEBUG
  void print_helper(
      size_t layer, std::set<const Node<RegexData, char_t> *> &traversed,
...
public:
  ...
  SubTree(std::vector<Node_T *> a, std::vector<Node_T *> b)
      : roots(a), leafs(b) {}
  SubTree() : roots(), leafs() {}
...
namespace matcher {
template <typename RegexData, typename char_t> class RegexMatcher {
  Node<RegexData, char_t> root;

  template <typename ConstIterator>
  static std::list<Limits>::iterator
  processLimit(const SubTree<Node<RegexData, char_t>> &,
               SubTree<Node<RegexData, char_t>> &, RegexData, ConstIterator &);

  template <typename ConstIterator>
  static SubTree<Node<RegexData, char_t>>
  processSet(std::vector<Node<RegexData, char_t> *>, RegexData,
             ConstIterator &);

  template <typename ConstIterator>
  static SubTree<Node<RegexData, char_t>>
  process(std::vector<Node<RegexData, char_t> *>, RegexData, ConstIterator &,
          ConstIterator, const bool);
...
public:
   ...
   *
   * @tparam Iterable Set of characters for the regex. Must implement
   * std::cbegin and std::cend
   * @tparam RegexData Value that will be associated with the regex
   */
  template <typename Iterable> void add_regex(Iterable, RegexData);

  /**
   * @brief Matches a string with all added regexes
   *
   * @tparam Iterable Set of characters. Must implement std::cbegin and
   * std::cend
   * @return std::vector<RegexData> Set of regexes' UIDs that match the text
   */
  template <typename Iterable> std::vector<RegexData> match(Iterable);
