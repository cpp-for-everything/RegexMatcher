...
namespace matcher
{
	...
	SubTree<Node<RegexData, char_t>> RegexMatcher<RegexData, char_t>::process(
		std::vector<Node<RegexData, char_t>*> parents, RegexData regex, ConstIterator& it, ConstIterator end, const bool inBrackets)
	{
	...
	template <typename RegexData, typename char_t> template <typename Iterable> void RegexMatcher<RegexData, char_t>::add_regex(Iterable str, RegexData uid)
	{
		auto it = std::cbegin(str);
		process(std::vector{&root}, uid, it, std::cend(str), false);
	}

	template <typename RegexData, typename char_t> template <typename Iterable> std::vector<RegexData> RegexMatcher<RegexData, char_t>::match(Iterable str)
	{
