...
std::list<Limits>::iterator RegexMatcher<RegexData, char_t>::processLimit(
    const SubTree<Node<RegexData, char_t>> &parent_of_latest,
  ...
  std::list<Limits>::iterator answer =
      Node<RegexData, char_t>::all_limits.insert(
          Node<RegexData, char_t>::all_limits.end(), Limits::common_edge);
  bool min = true;
  size_t number = 0;
...
SubTree<Node<RegexData, char_t>> RegexMatcher<RegexData, char_t>::processSet(
    std::vector<Node<RegexData, char_t> *> parents,
    [[maybe_unused]] RegexData regex, ConstIterator &it) {
  if (*it != '[') // not called at the beginning of a set
  {
  ...
  while (*it != ']' || takeTheNextSymbolLitterally) {
    if (!takeTheNextSymbolLitterally) {
      if (*it ==
          '\\') { // escape symbol is always followed by a reglar character
...
SubTree<Node<RegexData, char_t>> RegexMatcher<RegexData, char_t>::process(
    std::vector<Node<RegexData, char_t> *> parents, RegexData regex,
    ConstIterator &it, ConstIterator end, const bool inBrackets) {
  SubTree<Node<RegexData, char_t>> answer = {{}, {}};
  std::vector<SubTree<Node<RegexData, char_t>>> nodeLayers = {
      {parents, parents}};
  for (; it != end; it++) {
    if (*it == ')' && inBrackets) {
      break;
    ...
    if (*it == '[') { // start of a set
      ...
      SubTree<Node<RegexData, char_t>> newNodes =
          processSet(latest_parents.get_leafs(), regex, it);
      ...
      SubTree<Node<RegexData, char_t>> newLayer =
          process(nodeLayers.back().get_leafs(), regex, it, end,
                  true); // leaves it at the closing bracket
      nodeLayers.push_back(newLayer);
    } else if (*it == '|') {
      answer.roots.insert(answer.roots.end(), nodeLayers[1].get_leafs().begin(),
                          nodeLayers[1].get_leafs().end());
      answer.leafs.insert(answer.leafs.end(),
                          nodeLayers.back().get_leafs().begin(),
                          nodeLayers.back().get_leafs().end());
      nodeLayers.resize(1);
    } else if (*it == '{') {
      [[maybe_unused]] std::list<Limits>::iterator limits = processLimit(
          nodeLayers[nodeLayers.size() - 2], nodeLayers.back(), regex, it);
    } else if (auto special_regex =
                   Node<RegexData, char_t>::special_symbols.find(*it);
               special_regex !=
               Node<RegexData, char_t>::special_symbols.end()) {
      auto tmp_it = special_regex->second.cbegin();
      [[maybe_unused]] std::list<Limits>::iterator limits = processLimit(
          nodeLayers[nodeLayers.size() - 2], nodeLayers.back(), regex, tmp_it);
  ...
  answer.leafs.insert(answer.leafs.end(), nodeLayers.back().get_leafs().begin(),
                      nodeLayers.back().get_leafs().end());
  if (it == end) {
    Node<RegexData, char_t> *end_of_regex =
        new Node<RegexData, char_t>(symbol<char_t>::EOR);
    SubTree<Node<RegexData, char_t>> final_answer = {answer.get_roots(),
                                                     {end_of_regex}};
