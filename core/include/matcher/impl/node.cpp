...
std::vector<RegexData> common_values(const std::vector<RegexData> &sorted,
                                     const std::map<RegexData, T> &paths) {
...
template <typename RegexData, typename char_t>
void Node<RegexData, char_t>::merge(Node<RegexData, char_t> *with) {
  if (with == nullptr) {
    return;
  }
  ...
  for (auto incoming_neighbours : with->neighbours) {
    if (this->hasChild(incoming_neighbours.first)) {
      ...
      this->neighbours[incoming_neighbours.first].to->merge(
          incoming_neighbours.second.to);
    } else {
...
template <typename RegexData, typename char_t>
void Node<RegexData, char_t>::connect_with(
    Node<RegexData, char_t> *child, RegexData regex,
    std::optional<std::list<Limits>::iterator> limit) {
  if (auto existing_child = neighbours.find(child->current_symbol);
      existing_child != neighbours.end()) {
    if (auto it = existing_child->second.paths.find(regex);
        it != existing_child->second.paths.end()) {
      if (!it->second.has_value() && limit == std::nullopt) {
        it->second = Node<RegexData, char_t>::all_limits.insert(
            Node<RegexData, char_t>::all_limits.end(), Limits(1, 1));
      ...
      neighbours[child->current_symbol].paths.emplace(
          regex, Node<RegexData, char_t>::all_limits.insert(
                     Node<RegexData, char_t>::all_limits.end(), Limits(1, 1)));
    } else {
...
std::vector<RegexData> Node<RegexData, char_t>::match(ConstIterator begin,
                                                      ConstIterator end) {
...
template <typename RegexData, typename char_t>
template <typename ConstIterator>
std::vector<RegexData>
Node<RegexData, char_t>::match_helper(ConstIterator begin, ConstIterator end,
                                      const std::vector<RegexData> &paths,
                                      Node *prev) {
  if (begin == end) {
    if (auto it = this->neighbours.find(symbol<char_t>::EOR);
        it != this->neighbours.end()) {
      std::vector<RegexData> answer;
      std::vector<RegexData> potential_answer =
          common_values(paths, it->second.paths);
      if (prev != nullptr) {
  ...
  for (symbol<char_t> to_test :
       {current, symbol<char_t>::Any, symbol<char_t>::None}) {
    if (auto it = this->neighbours.find(to_test);
        it != this->neighbours.end()) {
      size_t ind = 0;
      ...
      for (const auto [pathId, limits_ptr] : it->second.paths) {
        if (limits_ptr.has_value() &&
            !limits_ptr.value()->is_allowed_to_repeat()) {
          continue;
        }
        if (prev != nullptr) {
          if (paths[ind] > pathId) {
      ...
      if (!new_paths.empty()) {
        ...
        for (auto match :
             it->second.to->match_helper(begin, end, new_paths, this)) {
...
void Node<RegexData, char_t>::print_helper(
  ...
  const std::basic_string<char_t> layer_str =
      (std::basic_stringstream<char_t>() << layer).str() + "_";
  const std::basic_string<char_t> next_layer =
      (std::basic_stringstream<char_t>() << (layer + 1)).str() + "_";
  ...
  for (auto child : neighbours) {
    if (nodes.find(child.second.to) == nodes.end()) {
      nodes.emplace(child.second.to,
                    next_layer + child.second.to->current_symbol.to_string());
    }
    std::cout << nodes[this] << " " << nodes[child.second.to] << " ";
    std::cout << child.second.paths.begin()->first
              << Limits::to_string(child.second.paths.begin()->second);
    for (auto it = std::next(child.second.paths.begin());
         it != child.second.paths.end(); it++) {
