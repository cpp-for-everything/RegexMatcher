...
namespace URLMatcher {
    ...
    private:
        ...
        static Container<std::pair<Limits, size_t>> all_limits;
         ...
         */
        void connect_with(Node<UniqueMatchDataPtr, char_t>* child, UniqueMatchDataPtr regex, std::optional<Container<std::pair<Limits, size_t>>::iterator> limits = std::nullopt);
        ...
        std::vector<UniqueMatchDataPtr> match(ConstIterator, ConstIterator) const;

        template<typename ConstIterator>
        std::vector<UniqueMatchDataPtr> match_helper(ConstIterator, ConstIterator, const std::vector<UniqueMatchDataPtr>&, const Node*, Map<std::pair<Limits, size_t>>&) const;
