...
private:
    ...
    struct char_t {
        ...
        bool is_any() const { return wildcard; }
...
public:
    ...
    void init(const ref_ptr regex_obj) {
        ...
        this->init(regex_obj->regex, regex_obj, 0, regex_obj->regex.size(), nullptr, &leafs);
        std::cout << "Generated leafs: " << leafs.size() << std::endl;
    }

    void init(const std::string& regex, const ref_ptr ref, size_t start, size_t end, node_ptr parent, std::vector<node_ptr>* leafs) {
        this->parent = parent;
        node_ptr current = this;
        // global leaf
            node_ptr leaf = new Node();
            leaf->endpoint = ref;
        // temporary elements needed for the capture
            node_ptr last_tmp = nullptr;
            std::vector<node_ptr> last_tmp_leafs;
        ...
        for (size_t i = start ; i < end ; i += 1) {
            if (regex[i] == '(') { // capturing group begins
                ...
                if (local_expression_end != end - 1) { // there's something after the capturing group so don't include it just yet
                    last_tmp = new Node();
                    last_tmp->init(regex, nullptr, i + 1, local_expression_end, current, &last_tmp_leafs); // skip opening and closing capturing group brakets
                }
                else { // there's nothing after the capturing group so just attach it to the current node
                    current->init(regex, ref, i + 1, local_expression_end, current, leafs); // skip opening and closing capturing group brakets
                }
                ...
                if (last_tmp != nullptr) { // the repeat operator is for a capture
                    // TODO: ...
                    last_tmp = nullptr;
                }
